...
export class GridLength {
    ...
    static parseString(value: string): 
        { length: GridLength, min?: number, max?: number }[]{
...
class ElementDef {
    ...
    measuredWidthFirstPass: boolean = false;
    measuredHeightFirstPass: boolean = false;
    cellTopOffset: number = 0;
    cellLeftOffset: number = 0;
}

@TypeId("ovuse.Controls.Grid")
export class Grid extends Panel implements ISupportCollectionChanged {

    private _rowDefs: RowDef[] | null = null;
    private _columnDefs: ColumnDef[] | null = null;
    private _elementDefs: ElementDef[] | null = null;
    private _lastDesiredSize: Size = new Size();
    ...
    //rows
    static rowsProperty = DependencyObject.registerProperty(
        Grid, "Rows", null, FrameworkPropertyMetadataOptions.AffectsMeasure | FrameworkPropertyMetadataOptions.AffectsRender, (v) => Grid.rowsFromString(v));
    ...
    //columns
    static columnsProperty = DependencyObject.registerProperty(
        Grid, "Columns", null, FrameworkPropertyMetadataOptions.AffectsMeasure | FrameworkPropertyMetadataOptions.AffectsRender, (v) => Grid.columnsFromString(v));
    ...
    //Grid.Row property
    static rowProperty = DependencyObject.registerProperty(
        Grid, "Grid#Row", 0, FrameworkPropertyMetadataOptions.AffectsMeasure, (v) => Grid.fromString(v));
    static getRow(target: DependencyObject): number {
        return <number>target.getValue(Grid.rowProperty);
    }
    static setRow(target: DependencyObject, value: number) {
        target.setValue(Grid.rowProperty, value);
    }

    //Grid.Column property
    static columnProperty = DependencyObject.registerProperty(
        Grid, "Grid#Column", 0, FrameworkPropertyMetadataOptions.AffectsMeasure, (v) => Grid.fromString(v));
    static getColumn(target: DependencyObject): number {
        return <number>target.getValue(Grid.columnProperty);
    }
    static setColumn(target: DependencyObject, value: number) {
    ...
    static rowSpanProperty = DependencyObject.registerProperty(
        Grid, "Grid#RowSpan", 1, FrameworkPropertyMetadataOptions.AffectsMeasure, (v) => Grid.spanFromString(v));
    static getRowSpan(target: DependencyObject): number {
        return <number>target.getValue(Grid.rowSpanProperty);
    }
    static setRowSpan(target: DependencyObject, value: number) {
        target.setValue(Grid.rowSpanProperty, value);
    }

    //Grid.ColumnSpan property
    static columnSpanProperty = DependencyObject.registerProperty(
        Grid, "Grid#ColumnSpan", 1, FrameworkPropertyMetadataOptions.AffectsMeasure, (v) => Grid.spanFromString(v));
    static getColumnSpan(target: DependencyObject): number {
        return <number>target.getValue(Grid.columnSpanProperty);
    }
    static setColumnSpan(target: DependencyObject, value: number) {
